...
export class CanvasLocal {
  ...
  drawBarra3d(x1: number, y1: number, x2: number, y2: number,
  x3:number, y3:number, x4:number, y4:number, x5:number, y5:number, x6:number, y6:number,color:string) {
  ...
  paint() {
    ...
    //let h: number[] = [10, 20, 30, 151];
    //let h: number[] = [100, 291, 543, 824, 1122];
    let maxEsc: number;
   ...
   /* let i=0;
    for(let x= 0; x < 8; x+=(8/(h.length*1)) ){
      this.graphics.strokeStyle = colors[i%5];
      if(i<h.length)
...
*/
    ...
    let i = 0;
    for (let y = 0.25; y <= 6; y += 1.35){
      this.drawLine(this.iX(0.6), this.iY(y), this.iX(8), this.iY(y));
      this.drawLine(this.iX(0), this.iY(y - 0.25), this.iX(0.6), this.iY(y));
      this.graphics.strokeText((maxEsc*i/4)+"",this.iX(-0.5), this.iY(y-0.3));
      i++;
    }
    this.graphics.strokeStyle = 'black';
    let ind = 0;
    let avance = (8/(h.length*2));
    for (let i = 0.5; i <=8; i += 2*avance){
      this.graphics.strokeStyle = colors[ind % colors.length];
      this.graphics.fillStyle = colors[ind  % colors.length];
      this.drawBarra3d(this.iX(i), this.iY(0), this.iX(i), 
                      this.iY(6 * h[ind] / maxEsc-0.1),
                      this.iX(i+0.5), this.iY((6 * h[ind] / maxEsc-0.1)+0.25), 
                      this.iX(i+avance+0.5), this.iY((6 * h[ind] / maxEsc-0.1)+0.25),
                      this.iX(i+avance+0.5), this.iY(0.25), 
                      this.iX(i+avance), this.iY(0),
                      colors[ind  % colors.length]); 
  
      this.graphics.strokeStyle = 'black';
      //console.log(this.rHeight*h[ind]/maxEsc)
      //estas tres lineas son el rectungulo que forma una barra
      this.drawLine(this.iX(i), this.iY(6 * h[ind] / maxEsc-0.1), this.iX(i), this.iY(0));
      this.drawLine(this.iX(i+avance), this.iY(6 * h[ind] / maxEsc-0.1), this.iX(i+avance), this.iY(0));
      this.drawLine(this.iX(i), this.iY(6 * h[ind] / maxEsc-0.1), this.iX(i+avance), this.iY(6 * h[ind] / maxEsc-0.1));
      //las siguientes lineas son l que ñe dan el efecto de profundidad
      this.drawLine(this.iX(i), this.iY(6 * h[ind] / maxEsc-0.1), this.iX(i+0.5), this.iY((6 * h[ind] / maxEsc-0.1)+0.25));
      this.drawLine(this.iX(i+0.5), this.iY((6 * h[ind] / maxEsc-0.1)+0.25),this.iX(i+avance+0.5), this.iY((6 * h[ind] / maxEsc-0.1)+0.25));
      this.drawLine(this.iX(i+avance), this.iY((6 * h[ind] / maxEsc-0.1)),this.iX(i+avance+0.5), this.iY((6 * h[ind] / maxEsc-0.1)+0.25));
      this.drawLine(this.iX(i+avance+0.5), this.iY((6 * h[ind] / maxEsc-0.1)+0.25),this.iX(i+avance+0.5), this.iY(0.25));
      this.drawLine(this.iX(i+avance), this.iY(0),this.iX(i+avance+0.5), this.iY(0.25));
    ...
    for (let y = 0; y< h.length; y++) {
      this.graphics.strokeText(""+h[y], this.iX(9), this.iY(5 - y));
